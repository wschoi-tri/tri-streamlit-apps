...
# URL 쿼리 파라미터 읽기 (초기 접속 / 새로고침 / URL 직접 공유 완벽 지원)
qp = st.query_params
...
# 0.1초 고속 비동기 SPA 자원 HTML/CSS/JS 템플릿
html_content = f"""
<!DOCTYPE html>
<html lang="ko">
<head>
...
<body>
    ...
    <script>
        ...
        // 브라우저 주소창 URL 갱신 함수 (새로고침 없이 안전하게 주소만 교체)
        function updateUrlQuery(kw, tab) {{
            try {{
                ...
                // 1. 부모 창 (window.parent) history.replaceState 시도
                try {{
                ...
                // 2. 최상위 창 (window.top) history.replaceState 시도
                try {{
                ...
                // 3. iframe 자체 history.replaceState (Fallback)
