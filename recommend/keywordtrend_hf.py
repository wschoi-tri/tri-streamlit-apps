...
# 4. API 캐싱 비동기 호출 함수 (0.05초 초고속 렌더링 지원)
...
# 5. URL 쿼리 파라미터 읽기 및 상태 동기화
qp = st.query_params
url_kw = qp.get("keyword", keywords_list[0] if keywords_list else "가디건")
if url_kw not in keywords_list:
    url_kw = keywords_list[0] if keywords_list else "가디건"
...
# 6. 좌측 사이드바 구성 (검색 & 키워드 선택)
st.sidebar.markdown(f"""
<div style="display:flex; align-items:center; justify-content:space-between; padding:8px 0 12px 0; border-bottom:1px solid #e2e8f0; margin-bottom:10px;">
    <span style="font-size:1.05rem; font-weight:800; color:#0f172a;">타겟 키워드 목록</span>
    <span style="font-size:0.75rem; background:#eff6ff; color:#2563eb; font-weight:700; padding:2px 8px; border-radius:10px;">{len(keywords_list)}</span>
</div>
""", unsafe_allow_html=True)
...
if not filtered_kws:
    st.sidebar.caption("일치하는 키워드가 없습니다.")
    current_kw = url_kw
else:
    current_kw_idx = filtered_kws.index(url_kw) if url_kw in filtered_kws else 0
    selected_kw = st.sidebar.radio(
        "키워드 선택",
        filtered_kws,
        index=current_kw_idx,
        label_visibility="collapsed",
        key="sidebar_kw_radio"
    )
    current_kw = selected_kw

# 키워드 변경 시 URL 파라미터 즉각 갱신 (실시간 URL 동기화 핵심)
if qp.get("keyword") != current_kw:
    st.query_params["keyword"] = current_kw
...
with col_ctrl:
    ...
    with c_site:
        site_label = "1 (하프클럽)" if url_site == "1" else "2 (보리보리)"
